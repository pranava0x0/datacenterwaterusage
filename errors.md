...
### [2026-07-02] Legislation theme grid rendered 0 for 5 of 6 cards — hardcoded tags drifted from the closed taxonomy
- **Module**: dashboard.py `LEGISLATION_THEME_DEFINITIONS` / `_build_legislation_themes_html`
- **Error**: No exception — silent data-quality failure. The legislation tab's 6-card theme grid showed `26 / 0 / 0 / 0 / 0 / 0` while the dataset actually held 22 cost-allocation, ~21 permit/moratorium, 11 preemptive-review, and 7 subsidy-condition bills. Caught by user review of a screenshot, not by tests.
- **Context**: Each theme card counted bills by matching a single hardcoded `tag` against `general_principles` tags. Five of the six hardcoded tags did not exist in the closed 13-value taxonomy (`LEGISLATION_PRINCIPLE_DESCRIPTIONS`) — three were invented labels ("Environmental Review", "Technology Mandate", "Consumer Protection"), one was a pure case mismatch ("Cost Allocation" vs "Cost allocation"), and one taxonomy concept spanned two tags the card ignored.
