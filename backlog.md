...
- **AWS Lake Anna VPDES individual permit (VA DEQ)** — added 2026-06-10 as a `datacenter` case (`AWS-LakeAnnaVA-VPDES-cooling-discharge-2026`). DEQ public hearing held 2026-06-09; final permit decision pending with no announced timeline. When DEQ decides: update the case's `outcome`, and if granted, the permit number becomes a *direct* hyperscaler NPDES permit to track via EPA ECHO DMR — the first one that isn't a receiving-WWTP proxy. *Monitor VA DEQ public notices / Town Hall for the final VPDES action.*
- **Quantum Loophole / Frederick MD AG enforcement** — added 2026-06-10 (`QuantumLoophole-FrederickMD-boring-discharges-2022-2024`). MDE rescinded its $130K settlement and referred the matter to the Maryland AG; watch for the filed enforcement action / consent decree and update `outcome` + penalty when it lands.
- **Microsoft Mount Pleasant WI wetland permit MKE 09-14** — final disposition not confirmed in available reporting (case flagged `cwa_applied: pending`); re-verify against WI DNR permit records and update the case.
