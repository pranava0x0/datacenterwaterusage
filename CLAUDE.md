...
- `data/reference/legislation.json` — 38 state/federal/local entries: bills (introduced/enacted/failed), agency rulemakings (OH EPA OHD000001 draft general permit), and major local zoning actions (Loudoun ZOAM 2025). Each enriched with timeline, recent news, public sentiment, and tagged general principles. Verified entries are flagged `verified: true`; lower-confidence entries carry a `status_detail` note to re-verify the bill number against the legislature's bill lookup. The June 2026 pass added the four 2026 state laws with newly **enacted** data-center water provisions (Idaho H 895 closed-loop/non-consumptive cooling mandate; South Dakota SB 135 "Data Center Bill of Rights," ≥10 MW cost-causation + water-supply compatibility; Utah HB 76 Water Transparency Amendments, projected + actual water disclosure at a lowered 10,000-sq-ft threshold; West Virginia HB 4983 Dept.-of-Commerce certification rule where binding water-protection amendments were *rejected*), Maryland's veto-overridden HB 270 / SB 116 impact study, the failed Washington HB 2515 and Indiana SB 79, and upgraded the federal Durbin transparency bill to its confirmed number **S. 4213**.
- `data/reference/cwa_investigations.json` — 63 cases across four categories: `datacenter` (14 direct hyperscaler / contractor CWA actions and §404 permit applications, anchored by the 2026 $20.5M Amazon Boardman nitrate settlement — the first eight-figure direct-hyperscaler water settlement — and including the Amazon/Walbridge New Carlisle IN §404/§401 wetlands cease-work; new June 2026 additions: Port of Morrow OR DEQ $727K fine at WWTP receiving DC wastewater, and Google §404 individual permit applications for Project Raspberry VA, Project Loch VA, and Port of Little Rock AR — the AR application is the largest §404 wetland footprint in any active DC permit at 16.8 acres), `adjacent` (6 — the xAI Colossus / Memphis greywater-plant case plus state groundwater-withdrawal and zoning/recharge-area fights, the QTS Fayette GA unbilled-water metering gap, and new: TransGas Adams Fork WV §404 citizen suit over unpermitted stream/wetland disturbance at a data-center power project), `industrial` (32 cooling-water, pretreatment, PFAS, POTW consent-decree cases including the 2026 Fort Smith AR §301(a) SSO consent-decree modification, and new: Greenidge Generation NY §316(a)/(b) thermal/intake violations at a bitcoin-mining/compute facility on Seneca Lake), and `precedent` (11 — Sackett, Maui, Loper Bright, SF v EPA, the Lewis v US 5th Circuit Sackett-application, plus Port of Tacoma citizen-suit scope and CERF v. Naples penalty-mootness). The CWA tab leads with a computed "What this record tells data centers" panel (`_cwa_datacenter_insights` / `render_cwa_datacenter_insights`, computed over the `datacenter` subset) that surfaces the permittee-shield and construction-stormwater patterns and frames the operational exposure as the *receiving WWTP's* NPDES permit — the permits the pipeline already tracks via EPA ECHO.
