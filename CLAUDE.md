...
- **Dashboard (deployed)**: a **pre-rendered static site** with **no third-party assets at all**. `build_site.py` imports `dashboard`'s pure `_build_*_html` builders + data constants and emits `pages/index.html` (vanilla-JS tabs/filters/collapsibles; a sticky outline-chip tab strip, collapsible card groups and a back-to-top control — see DESIGN.md §4a). This replaced the old stlite/Pyodide WASM deploy in June 2026 — first paint went from ~25–40 s to ~35 ms. Edit a card builder or the data and both the Streamlit app and the static site change together; regenerate with `python build_site.py`, which writes **three** files into `pages/`:
  - **`index.html`** (~1.5 MB; a build test trips at 1.6 MB).
  - **`graph-data.json`** (~620 KB) — the Explore connection graph + TF-IDF index, split out of `index.html` on 2026-08-24 (page was 2.11 MB). The Explore tab `fetch`es it same-origin **on first activation only**, with a loading state and a plain failure message; a reader who never opens Explore never downloads it. `dashboard._build_explore_html(payload_src)` takes the URL — pass `None` (the default) and the payload inlines instead, which is what the Streamlit surface needs since a `components.html` iframe has no origin to fetch from.
...
- **Testing**: pytest + pytest-asyncio (798 tests)
